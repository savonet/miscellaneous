...
\end{verbatim}
In this case, the card we want to use is: device \verb+0+, subdevice \verb+0+, thus:
\verb+hw:0,0+. We now create a file \verb+/etc/asound.conf+ (or \verb+~/.asoundrc+ for single-user
...
\end{verbatim}
This creates a new alsa device that you can use with liquidsoap. The \verb+plug+ operator
...
\end{verbatim}
In the second case -- disabling pulseaudio, you can edit the file \verb+/etc/pulse/client.conf+ and
